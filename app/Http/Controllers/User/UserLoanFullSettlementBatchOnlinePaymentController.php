...
final class UserLoanFullSettlementBatchOnlinePaymentController extends Controller
{
    ...
    public function start(
        ...
        $quote = $portalPresenter->fullSettlementQuoteForAllOpenFiles($customer);
        if ($quote === null) {
            return $this->backWithPayFlash(false, 'تسویهٔ کلی برای پرونده‌های باز در دسترس نیست.');
        }
        ...
        $description = 'تسویه کلی همه پرونده‌ها';
        $callbackUrl = route('payment.zibal.callback', absolute: true);
        $orderId = 'fsb-'.$customer->id.'-'.time();

        $intent = CustomerLoanFullSettlementBatchOnlinePaymentIntent::query()->create([
            ...
            'items_json' => $quote['files'],
            'track_id' => null,
            'status' => CustomerLoanFullSettlementBatchOnlinePaymentIntent::STATUS_CREATED,
            'gateway_key' => 'zibal',
            'zibal_ref_number' => null,
            'failure_reason' => null,
        ]);
        $ledger->syncFromFullSettlementBatchIntent($intent);
        ...
        if (! $req['ok'] || $req['track_id'] === null) {
            ...
            $ledger->syncFromFullSettlementBatchIntent($intent->fresh());
        ...
        DB::transaction(function () use ($intent, $req): void {
            $fresh = CustomerLoanFullSettlementBatchOnlinePaymentIntent::query()
                ->whereKey($intent->id)
                ->lockForUpdate()
                ->first();
            if ($fresh === null) {
            ...
            if ($fresh->status !== CustomerLoanFullSettlementBatchOnlinePaymentIntent::STATUS_CREATED) {
                return;
            }
            $fresh->update([
                'track_id' => $req['track_id'],
                'status' => CustomerLoanFullSettlementBatchOnlinePaymentIntent::STATUS_REDIRECTED,
            ]);
        });
