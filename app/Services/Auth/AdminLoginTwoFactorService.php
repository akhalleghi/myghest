...
final class AdminLoginTwoFactorService
{
     ...
     */
    public function beginChallenge(Admin $admin, Request $request, bool $remember): array
    {
        $mobile = IranMobile::normalize((string) ($admin->mobile ?? ''));
        if ($mobile === null) {
        ...
        $result = $this->rawSms->send($mobile, $this->buildSmsBody($plain), 'admin-login-2fa');
        if (! ($result['ok'] ?? false)) {
    ...
    /**
     * @return array{admin: Admin, remember: bool}
     */
    public function verify(string $loginSession, string $code, Request $request): array
    {
        ...
        $admin = Admin::query()->whereKey((int) ($data['admin_id'] ?? 0))->first();
    ...
    {
        return 'admin_login_2fa:'.hash('sha256', $sessionId);
    }
    ...
    private function buildSmsBody(string $plainCode): string
    {
        ...
        return 'کد ورود مدیر به '.$appName.chr(10)
            .$plainCode.chr(10)
    ...
    {
        return 'admin-login-2fa-resend|'.sha1((string) $request->ip().'|'.$sessionId);
    }

    private function verifyThrottleKey(Request $request, string $sessionId): string
    {
        return 'admin-login-2fa-verify|'.sha1((string) $request->ip().'|'.$sessionId);
    }
